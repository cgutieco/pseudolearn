...
`apps/pseudolearn_app/README.md` para el cliente Flutter y `apps/fe-pseudolearn/README.md` para el sitio
público.

**Estado:** Monorepo de cuatro miembros activos —dos paquetes bajo `packages/` y dos aplicaciones bajo
...
- **CI/CD centralizado en la raíz:** No hay configuración de integración continua en este repositorio.
  La verificación mecánica de cada miembro se ejecuta localmente con los comandos declarados en su
  propio `README.md` y reunidos en §2.5.
...
- **Entorno de ejecución y SDKs:** Dart SDK `^3.5.0` (ambos paquetes Dart); Flutter SDK `>=3.24.0`
  (requerido únicamente por `apps/pseudolearn_app`); Node `>=22.12.0` con `pnpm` (requerido únicamente
  por `apps/fe-pseudolearn`); Python `>=3.9` con `fonttools` (requerido únicamente por
...
# Sitio público: formato, lints, tipos, verificadores estáticos, tests y presupuesto
(cd apps/fe-pseudolearn && pnpm check:all)
```
...
- **`apps/` — lo que se despliega a una persona.** El cliente Flutter y el sitio público.
...
núcleo. La otra es de generación: el motor de marca escribe artefactos que sus consumidores versionan, y
ninguno de ellos lo necesita para compilar.

```
┌──────────────────────────┐        ┌──────────────────────────┐
│ apps/pseudolearn_app     │        │ apps/fe-pseudolearn      │
│ (cliente Flutter)        │        │ (sitio público, Astro)   │
└─────────┬────────────────┘        └───────────┬──────────────┘
...
└─────────────────────────────┘                 │
                                                │
          ▲ genera y verifica                   ▲ genera y verifica
          │                                     │
┌─────────┴─────────────────────────────────────┴──────────────┐
│ packages/pseudolearn_brand                                    │
│ (motor de la identidad visual, Python)                        │
└───────────────────────────────────────────────────────────────┘
```
...
| `apps/fe-pseudolearn/`        | Sitio público: landing, contacto y soporte, e historial de versiones                                                   | Paquetes de su propio `package.json`; el módulo de marca que `pseudolearn_brand` emite; capas internas según su propio `architecture.yaml` | Símbolos de `packages/pseudolearn_core`; geometría de marca escrita a mano  |
...
- **Alternativas descartadas y por qué:**
    - *`melos`:* Aporta ejecución de comandos en paralelo sobre N paquetes y versionado sincronizado.
      Solo entiende paquetes de Dart, con lo que dejaría fuera a la mitad del monorepo; y sobre la mitad
      que sí cubre, el paralelismo no ahorra tiempo apreciable con una dependencia unidireccional, y el
      versionado sincronizado no aplica porque el núcleo no se publica.
    - *Workspaces de un gestor de paquetes ajeno al ecosistema Dart/Flutter (npm, yarn):* Fuera del
      ecosistema del lenguaje de ambos miembros; introduciría una segunda cadena de herramientas sin
      beneficio.
...
- **Problema:** `pseudolearn_core`, `apps/pseudolearn_app` y `apps/fe-pseudolearn` cambian por
  razones distintas, pero el público necesita un solo número con el que hablar del producto, y el
  repositorio necesita que ninguna publicación del núcleo se quede sin llegar a nadie. Una versión
  compartida por los tres obliga a subir el núcleo cada vez que se corrige una errata de la web y
  miente sobre lo que cambió; tres versiones sueltas no obligan a nada y dejan que un núcleo nuevo se
  quede sin app que lo lleve.
- **Elección:** tres números con dueños distintos y una sola regla que ata dos de ellos.
    ...
    - `apps/pseudolearn_app` lleva la **versión de producto**: la de la tienda, la que publica el sitio
      y la que se pide al reportar un fallo.
    - `apps/fe-pseudolearn` lleva la suya y no entra en la regla: publicar una errata en la web no
      publica una versión del producto.
...
- **Unidad de publicación:** el par `(app, núcleo)`. Toda publicación se declara como una entrada con
  la versión de producto, la del núcleo anclado, la fecha y los identificadores de lo que trae. De esa
  entrada sale lo que el sitio muestra, de modo que no hay una segunda lista que pueda divergir.
- **Alternativas descartadas y por qué:**
    - *Versión única compartida por los tres miembros:* cada cambio de interfaz o de copia obligaría a
      subir la versión del núcleo, que es un contrato: el número dejaría de significar nada.
    - *Tres versiones completamente independientes:* no obliga a nada, y la pregunta que el proyecto
      necesita poder contestar —«¿con qué motor se probó esta app?»— se queda sin respuesta declarada.
...
- **Consecuencia:** el criterio para ubicar un archivo se vuelve una pregunta sobre su papel, que es
  estable, en vez de una sobre su lenguaje, que no lo es —el monorepo ya sostiene Dart, TypeScript y
  Python—. A cambio, `packages/` deja de tener una cadena de herramientas única: no hay un comando que
  prepare o pruebe todos sus miembros a la vez, y cada uno declara los suyos en su `README.md` §2. Eso ya
  era cierto entre `apps/pseudolearn_app` y `apps/fe-pseudolearn`, así que no introduce una situación
  nueva, solo la extiende.
- **Alternativas descartadas y por qué:**
...
`apps/pseudolearn_app/architecture.yaml` y `apps/fe-pseudolearn/architecture.yaml` en YAML;
...
| Verificación del sitio       | `cd apps/fe-pseudolearn && pnpm check:all`                                    | Formato, lints, tipos, capas, i18n, comentarios, marca pegada a mano, tamaño, tests y presupuesto                                                 | `apps/fe-pseudolearn/architecture.yaml`           |
